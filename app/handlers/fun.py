...
from app.utils.decorators import personal_limit, general_chat_only
from app.core.config import bot_settings
from telegram import Update
from app.services import predictions, jokes
...
async def bless(update: Update, context: CallbackContext):
    ...
    chat_admins = await context.bot.get_chat_administrators(chat_id=bot_settings.chat_id)
...
async def joke(update: Update, context: CallbackContext):
    await update.message.reply_text(
        ...
        f"{await jokes.read_random_joke()}"
    )
...
async def weather(update: Update, context: CallbackContext):
    url = f"http://api.openweathermap.org/data/2.5/weather?q=Kosice&appid={bot_settings.weather_api_key}&units=metric&lang=sk"
