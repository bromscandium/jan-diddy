...
async def is_possible(update, required_permission: str, use_grant_admins=False) -> bool:
    allowed_ids = bot_settings.grant_admin_ids if use_grant_admins else bot_settings.admin_ids
    ...
    if user.id in bot_settings.admin_ids:
...
async def reset_warn(update: Update, context: CallbackContext) -> None:
    ...
    await context.bot.send_message(chat_id=bot_settings.chat_id, text="Všetky varovania boli automaticky resetované!")
